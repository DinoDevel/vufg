...
with open('vendor_ids.txt', 'rt', encoding='utf-8') as file:
...
def is_range_arguments_correct(range_from: int, range_to: int):
    if range_from < range_min_limit:
        print(f"""vufg: error: range must start at a value greater than 0x{
                format(range_min_limit, '04x')}.""")
        return False
    elif range_from > range_to:
        print("vufg: error: range end must be greater than range start.")
        return False
    elif range_to > range_max_limit:
        print(f"""vufg: error: range must end at a value lesser than 0x{format(
                range_max_limit, '04x')}.""")
        return False
    return True
...
def add_filter(filter_id):
    ...
    device_filter = ET.SubElement(elem_device_filters, "DeviceFilter",
                                  name="Filter 0x" + hexcount, active="true",
...
generate filters for every VID within range, including invalid or obsolete. If
omitted (by default) generate filters including only valid VIDs within range.
Warning! Any existing filters will be overwritten.''')
parser.add_argument('-f', '--from', dest='r_from', type=auto_int,
                    metavar='value', default=range_min_limit,
                    help=f'''generate filters within range of VIDs beginning
                    from 'value' (0x{format(range_min_limit, '04x')
                    }-0x{format(range_max_limit, '04x')} hex or dec, default
                    0x{format(range_min_limit, '04x')})''')
parser.add_argument('-t', '--to', dest='r_to', type=auto_int, metavar='value',
                    default=range_max_limit, help=f'''generate filters within r
                    ange of VIDs ending at 'value' (0x{format(range_min_limit
                    ,'04x')}-0x{format(range_max_limit, '04x')} hex or dec,
                    default 0x{format(range_max_limit, '04x')})''')
parser.add_argument('filename', type=argparse.FileType('r+'), help='VirtualBox\
